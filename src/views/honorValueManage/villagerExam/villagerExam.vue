...
<template>
  <div class>
    ...
    <el-table border v-loading="loading" :data="tableListData" style="width: 100%">
      ...
      <el-table-column prop="name" label="申请者"></el-table-column>
      <el-table-column prop="createTime" label="提交时间"></el-table-column>
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <div>
            <el-button type="text" @click="handleView(scope.row, 2)&&isAuth('honor:single:audit')" v-if="scope.row.state == '1'">审核</el-button>
            <el-button type="text" v-if="isAuth('honor:single:view')" @click="handleView(scope.row, 1)">查看</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    async getListData() {
      ...
      var objcommon = {
        ...
        // member_id:'20201601274810389'
        // memberId: this.adminUserInfo.memberId
      };
      const res = await this.$http.personalList({
        ...objcommon,
        ...this.obj,
      });
      if (res.status === "success") {
