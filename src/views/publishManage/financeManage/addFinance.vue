  ...
  <div class>
    <el-form ref="editForm" :model="editForm" :rules="editRules" label-width="100px" style="width:60%">
      ...
      <el-form-item label="发布人：" prop="creator" >
        <el-input v-model="editForm.creator" placeholder="请输入20字以内的发布人姓名"></el-input>
      </el-form-item>
      ...
      <el-form-item label="金融内容：" class="editor-item-con" prop="details">
        <el-input v-show="false" type="textarea" minlength="0" maxlength="2000" :rows="10" v-model="editForm.details" placeholder></el-input>
        <!-- 编辑器　替换　-->
        <!-- 图片上传组件辅助-->
        <el-upload class="avatar-uploader" :action="upLoadApiUrl" accept="image/jpeg, image/jpg, image/gif, image/png" :data="uploadImgData" :show-file-list="false" :on-success="uploadSuccess" :on-error="uploadError"></el-upload>
  ...
  methods: {
    ...
    async getListData() {
      ...
      if (res.status === "success") {
        ...
        this.editForm.title = resData.title;
        this.editForm.picListUrls = 1;
        this.editForm.details = resData.context;
        this.editForm.creator=resData.creator
        this.fileImgList = [
          {
            url: resData.picUrl,
            name: "1",
    ...
    async submitForm(formName) {
      ...
      if (this.editForm.title.length <= 0 || this.editForm.title.length > 20) {
        this.$message({
          type: "warning",
          message: "请输入20字以内的主题名称",
        });
      } else if (this.editForm.creator.length <= 0) {
        this.$message({
          type: "warning",
          message: "请输入20字以内的发布人姓名",
        });
        ...
        this.editForm.details.length <= 0 ||
        this.editorWordLength > 2000
      ) {
        this.$message({
          type: "warning",
          message: "请输入2000字以内的金融内容",
          ...
          if (valid) {
            this.$alert("确定要发布该金融信息么?", "发布确定", {
              ...
              .then(async () => {
                if (this.id) {
                  this.isdisable = true;
                  const res = await this.$http.getFinanceUpdate({
                    id: this.id,
                    title: this.editForm.title,
                      creator: this.editForm.creator,
                    context: this.editForm.details,
                  ...
                  this.isdisable = true;
                  const res = await this.$http.getFinanceInsert({
                    title: this.editForm.title,
                    context: this.editForm.details,
                    picUrl: this.fileImgList[0].url
                      ? this.fileImgList[0].url
