<template>
    <div>
        ...
        <el-form v-if="memberChange" :model="memberChange" label-width="100px"  :rules="rulesChange" ref="memberChangeRef">
            ...
            <el-form-item label="职务：" prop="positionName">
                <el-input v-model="memberChange.positionName" placeholder="请输入2~12个字的职务名称" style="width: 300px;margin-right: 50px"></el-input>
            </el-form-item>
            <el-form-item label="联系方式：" prop="positionPhone">
                <el-input v-model="memberChange.positionPhone" placeholder="请输入联系电话" style="width: 300px;margin-right: 50px"></el-input>
            </el-form-item>
            <el-form-item label="角色选择：" prop="type">
                <el-checkbox-group v-model="memberChange.type">
                    <el-checkbox v-for="(item,index) in memberChange.typeRole" :key="index" :label="item.id" name="type">{{item.roleName}}</el-checkbox>
                </el-checkbox-group>
            </el-form-item>
        </el-form>
        <div class="userHeader">账号信息</div>
        <ul class="userList">
            <li><label>用户名：</label><span>{{memberChange.account}}</span></li>
            <!--<li><label>用户ID：</label><span>{{memberChange.userName}}</span></li>-->
            <li><label>绑定手机号：</label><span>{{phoneNumFilterLogin(memberChange.mobile) || '暂未绑定'}}</span></li>
            <!--<li><label>绑定微信号：</label><span>{{memberChange.mobile}}</span></li>-->
            <li><label>注册时间：</label><span>{{memberChange.createTime}}</span></li>
        </ul>

        <div style="width: 400px;text-align: center">
            <el-button type="primary" @click="changeForm('memberChangeRef')" style="margin-right: 30px">保存修改</el-button>
            <!--<el-button type="danger" @click="resetPwd('memberChangeRef')">重置密码</el-button>-->
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            async changeForm(){
                ...
                /*if(2<=this.memberChange.positionName.length<=12&&this.memberChange.positionPhone&&this.memberChange.type){
                ...
                if(this.memberChange.positionName.length<2 || this.memberChange.positionName.length>12){
                    ...
                    if(res.status==='success'){
                        ...
                        setTimeout(() => {
                            this.$router.push({name: 'moreOrganization'});
                        }, 1000);
                    }else{
                        this.$message({
                            type: 'error',
